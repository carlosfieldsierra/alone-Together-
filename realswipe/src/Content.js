import React from "react";
import { Image, StyleSheet, Text, View,TouchableOpacity } from "react-native";
const styles = StyleSheet.create({
  title1: {
    fontSize: 48,
    fontWeight: "300",
    fontFamily:'NunitoExtraBold'
  },
  title2: {
    fontSize: 48,
    fontWeight: "600",
    fontFamily:'NunitoExtraBold'
  },
  description: {
    opacity: 0.5,
    fontSize: 16,
    fontFamily:'NunitoExtraBold'
  }
});



export default ({
  color,
  backgroundColor,
  source,
  title1,
  title2,
  navigation
}) => {
  return (
    <View
      style={{
        ...StyleSheet.absoluteFillObject,
        padding: 32,
        backgroundColor,
        alignItems: "center",
        justifyContent: "space-around"
      }}
    >
      <View style={{flexDirection:'row',marginTop:20}} >
        <Text style={{fontFamily:'NunitoExtraBold', fontSize: 20,color,}}>How To Play</Text>
        <TouchableOpacity onPress={()=>{navigation.navigate('loadingscreen')}}>
          <Text style={{fontFamily:'NunitoExtraBold',marginLeft:180, fontSize: 20,color,}}> Skip</Text>
        </TouchableOpacity>
      </View>
      <View style={{flex:1,justifyContent:'center',alignItems:'center'}}>
        <Image {...{ source }} />
        <View>
          <Text style={[styles.title1, { color }]}>{title1}</Text>
          <Text style={[styles.title2, { color }]}>{title2}</Text>
          <Text style={[styles.description, { color }]}>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec rutrum
            pharetra pellentesque. Donec blandit purus ut arcu vulputate, at
            rutrum sem dictum. Mauris sagittis felis interdum arcu ultrices
            vestibulum.
          </Text>
        </View>
      </View>
    </View>
  );
};
